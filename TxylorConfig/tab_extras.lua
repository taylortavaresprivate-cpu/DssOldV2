...
function M.draw()
	...
	if cfg.scroll_gas_enabled then
		...
		local modeLabel = SCROLL_MODE_NAMES[mode] or 'Ambos'

		-- Hint dinâmica de acordo com modo + inversão
		...
		u.cfgSlider('Decay', 'scroll_gas_decay', 0.0, 2.0, '%.2f',
			'Decaimento por segundo em direção ao zero.\n0.0 = mantém | 0.5 = decai em ~2s | 2.0 = rápido')
		u.hint('0.0 = mantém | >0 = volta ao neutro sozinho.')
		...
		if cfg.scroll_gas_mode == 0 then
			-- Só gás: barra simples da esquerda
		...
		elseif cfg.scroll_gas_mode == 1 then
			-- Só freio: barra simples da esquerda
			ui.drawRectFilled(cur, cur + vec2(barW, 10), rgbm(0.12, 0.12, 0.12, 1), 2)
			...
			-- Ambos: barra bidirecional com centro
			local cx = barW / 2
			ui.drawRectFilled(cur, cur + vec2(barW, 10), rgbm(0.12, 0.12, 0.12, 1), 2)
